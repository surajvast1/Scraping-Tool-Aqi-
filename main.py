...
def main():
    ...
    print(f"cpcb:fetch {time.perf_counter() - t0:.3f}s")

    # ---------------- COMPUTE NEAREST ----------------
    print("Computing nearest station...")
    t1 = time.perf_counter()

    # with open("cpcb.json", "w") as f:
    #     json.dump(response.json(), f)

    # print("Top-level keys:", response.json().keys())
    # print("Number of states:", len(response.json().get("country", [])))
    # print("Number of cities:", len(response.json().get("country", [])[0].get("citiesInState", [])))
    # print("Number of stations:", len(response.json().get("country", [])[0].get("citiesInState", [])[0].get("stationsInCity", [])))
